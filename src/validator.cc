...
Result Validator::OnControlExpr(ControlExpr* expr) {
  ...
  const Func* callee;
  if (Succeeded(CheckFuncVar(&expr->var, &callee))) {
    typechecker_.OnControl(callee->decl.sig.param_types,
                        callee->decl.sig.result_types);
  }
  return Result::Ok;
}
...
Result Validator::OnContinuationDeleteExpr(ContinuationDeleteExpr* expr) {
  expr_loc_ = &expr->loc;
  typechecker_.OnContinuationDelete();
  return Result::Ok;
}
